function mobile_finder_cosine(mobile_feat, mobiles, mobile_name)
    names = mobiles.device_brand;
    index = find(ismember(names, mobile_name));
    [nmobiles, ~] = size(mobile_feat);
    similarity = size(nmobiles);
    for i = 1:nmobiles
        if i ~= index
            sim = cosine_similarity(mobile_feat(i, :), mobile_feat(index, :));
            similarity(i) = sim;
        end
    end
